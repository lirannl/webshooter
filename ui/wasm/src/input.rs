...
pub fn setup_keyboard(canvas: &HtmlCanvasElement) {
    ...
    let click_cb = Closure::wrap(Box::new(move || {
        let _ = canvas_click.request_fullscreen();
    }) as Box<dyn FnMut()>);
...
pub fn setup_touch(canvas: &HtmlCanvasElement) {
    ...
    let press_cb = Closure::wrap(Box::new(move |e: TouchEvent| {
        e.prevent_default();
        let rect = canvas_press.unchecked_ref::<Element>().get_bounding_client_rect();
        ...
        for (slot, x, y) in touches_vec {
            let msg = ClientDatagram::TouchscreenMulti {
                touches: vec![(slot, x, y)],
            };
            let bytes = msg.to_bytes();
            let buf = js_sys::Uint8Array::from(&bytes[..]);
            with_wt(|gwt| {
    ...
    let _ = canvas.add_event_listener_with_callback("touchstart", press_cb.as_ref().unchecked_ref());
    ...
    let release_cb = Closure::wrap(Box::new(move |e: TouchEvent| {
        ...
        for i in 0..touches.length() {
            ...
            if let Some(slot) = slots_rel.borrow_mut().remove(&id) {
                ...
                let rect = canvas_release.unchecked_ref::<Element>().get_bounding_client_rect();
                let cw = canvas_release.width() as f64;
                let ch = canvas_release.height() as f64;
                let nx = (touch.client_x() as f64 - rect.left()) / rect.width();
                let ny = (touch.client_y() as f64 - rect.top()) / rect.height();
                let x = clamp((nx * cw).round(), 0.0, cw - 1.0) as u16;
                let y = clamp((ny * ch).round(), 0.0, ch - 1.0) as u16;
                release_vec.push((slot, x, y));
            }
        }
        if !release_vec.is_empty() {
            let msg = ClientDatagram::TouchscreenMulti {
                touches: release_vec.clone(),
            };
            let bytes = msg.to_bytes();
            let buf = js_sys::Uint8Array::from(&bytes[..]);
            with_wt(|gwt| {
